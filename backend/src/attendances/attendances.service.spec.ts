...
describe('AttendancesService', () => {
  ...
  describe('clockIn', () => {
    ...
    it('should throw BadRequestException if already clocked in today', async () => {
      ...
      await expect(service.clockIn(1, 'photo.jpg')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw NotFoundException if employee does not exist', async () => {
      ...
      await expect(service.clockIn(999, 'photo.jpg')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
