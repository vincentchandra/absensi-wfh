...
describe('AttendancesController', () => {
  ...
  describe('clockIn', () => {
    it('should delegate to service with employeeId, filename, and notes', async () => {
      const mockPhoto = { filename: 'attendance-123.jpg' } as Express.Multer.File;
      ...
      expect(service.clockIn).toHaveBeenCalledWith(5, 'attendance-123.jpg', 'WFH today');
  ...
  describe('clockOut', () => {
    it('should delegate to service with employeeId', async () => {
      service.clockOut!.mockResolvedValue({ id: 1, clockOut: '17:00:00' } as any);
